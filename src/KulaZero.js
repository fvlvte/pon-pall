...
export const KulaZero = ({
  podstawaGrawitacji,
  modyfikatorPrędkości,
  limitPrędkości,
  początkoweKule,
  maxKule,
  przyrostKul,
      ...
      for (const kulson of poprzednieKulsony) {
        if (kulson.isClickt) {
          if (currentTime < kulson.whenSpawn) {
            ...
            kulson.x =
              Math.random() * (width - 2 * kulson.scale) + kulson.scale;
            kulson.velocity = 1;
          }
        } else if (secsElapsedPct > 0) {
          kulson.y += podstawaGrawitacji * secsElapsedPct * kulson.velocity;
          kulson.obrutPapieza +=
            ((podstawaGrawitacji * kulson.velocity) / 2) * secsElapsedPct;

          if (kulson.obrutPapieza > 360) {kulson.obrutPapieza -= 360;}
        ...
        return (
          <TouchableOpacity
            ...
            onPress={e => {
              if (
                isPointInCircleInBox(
                  ...
                  {x: 0, y: 0, width: kulson.scale, height: kulson.scale},
                  {
                    x: kulson.scale / 2,
                    y: kulson.scale / 2,
                    radius: kulson.scale / 2,
                  },
                )
              ) {
                ustawKulsony(kulsony => {
                  kulsony[kulsony.indexOf(kulson)].isClickt = true;
                  kulsony[kulsony.indexOf(kulson)].whenSpawn =
                    new Date().getTime() + Math.random() * 5000 + 1000;
                  return [...kulsony];
                });
            ...
            <ObrazTło
              ...
              source={require('./assets/rareSkin.png')}
              resizeMode="cover"></ObrazTło>
          </TouchableOpacity>;
        );
      })}
    </Widok>
